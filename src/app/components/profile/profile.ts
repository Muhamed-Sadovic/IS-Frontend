...
export class ProfileComponent implements OnInit {
  ...
  saveProfile() {
    if (!this.profileForm.ime || !this.profileForm.prezime) {
      Swal.fire('Pažnja', 'Ime i prezime ne smeju biti prazni.', 'warning');
      return;
    }
  ...
  changePassword() {
    if (this.passForm.novaLozinka.length < 6) {
      Swal.fire('Pažnja', 'Nova lozinka mora imati najmanje 6 karaktera.', 'warning');
      return;
    }
